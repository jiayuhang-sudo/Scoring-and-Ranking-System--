...
void JudgeScoring(vector<Player>& vTemp)
{
    for (auto& p : vTemp)
    {
        for (int j = 0; j < JUDGE_NUM; j++)
        {
            ...
            p.mAllScore.push_back(temp);//第三处错误: 容器初始化为空, 需要用push_back添加, 不能直接用下标
        }
    }
...
{
    for (auto& p : vTemp)//int i = 0; i < PLAYER_NUM; i++
    {
        sort(p.mAllScore.begin(), p.mAllScore.end());
        double TotalScaore = 0.0;//规范: double类型初始化用0.0
        for (int j = 1; j < JUDGE_NUM - 1; j++)
        {
            TotalScaore += p.mAllScore[j];//第四处错误: 下标写错(尽量避免)
        }
        p.mAverageScore = TotalScaore / (JUDGE_NUM - 2);
    }
